...
__all__ = ['GroupedArray', 'transform_series', 'build_transform_name', 'TimeSeries']
...
class TimeSeries:
    ...
    def _transform(
        ...
        features = self.compute_transforms()
    ...
    def fit_transform(
        ...
        if not df.index.is_monotonic_increasing:
            df = df.sort_index()
        self._fit(df, static_features)
        return self._transform(df, dropna, keep_last_n)
    ...
    def predict(self, model, horizon: int, **kwargs) -> pd.DataFrame:
        self._predict_setup()
        # this avoids installing xgboost only to check if the model is instance of xgb.Booster
        model_is_xgb_booster = (
            type(model).__module__ == 'xgboost.core'
            and type(model).__name__ == 'Booster'
        )
        for _ in range(horizon):
            new_x = self.update_features()
            if model_is_xgb_booster:
                import xgboost as xgb

                new_x = xgb.DMatrix(new_x)
            predictions = model.predict(new_x)
            self.update_y(predictions)
