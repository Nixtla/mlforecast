...
def _rescale_interval_columns(
    ...
    """Multiply interval half-widths by per-row σ̂_tgt, one assignment per column.
    ...
    for model_name in model_names:
        ...
        for lv in level:
            for direction in ("lo", "hi"):
                ...
                offset = fcst_df[col].to_numpy().astype(float) - mean_arr
                fcst_df = ufp.assign_columns(
                    fcst_df, col, mean_arr + offset * sigma_tgt
                )
    return fcst_df


def _rescale_interval_columns_single_call(
    fcst_df: DFType,
    model_names: List[str],
    level: List[Union[int, float]],
    sigma_tgt: np.ndarray,
) -> DFType:
    """Single assign_columns call: collect all interval cols into one matrix."""
    fcst_df = ufp.copy_if_pandas(fcst_df, deep=False)
    n_rows = len(sigma_tgt)
    n_level_dirs = 2 * len(level)
    n_cols = len(model_names) * n_level_dirs
    all_cols: List[str] = []
    means_matrix = np.empty((n_rows, n_cols), dtype=float)
    col_idx = 0
    for model_name in model_names:
        mean_arr = fcst_df[model_name].to_numpy().astype(float)
        for lv in level:
            all_cols.append(f"{model_name}-lo-{lv}")
            all_cols.append(f"{model_name}-hi-{lv}")
        means_matrix[:, col_idx : col_idx + n_level_dirs] = mean_arr[:, np.newaxis]
        col_idx += n_level_dirs
    vals_matrix = np.column_stack(
        [fcst_df[col].to_numpy().astype(float) for col in all_cols]
    )
    rescaled = means_matrix + (vals_matrix - means_matrix) * sigma_tgt[:, np.newaxis]
    return ufp.assign_columns(fcst_df, all_cols, rescaled)


def _rescale_interval_columns_per_model(
    fcst_df: DFType,
    model_names: List[str],
    level: List[Union[int, float]],
    sigma_tgt: np.ndarray,
) -> DFType:
    """One assign_columns call per model: collapse level×direction into a matrix."""
    fcst_df = ufp.copy_if_pandas(fcst_df, deep=False)
    for model_name in model_names:
        mean_arr = fcst_df[model_name].to_numpy().astype(float)
        cols = [
            f"{model_name}-{d}-{lv}" for lv in level for d in ("lo", "hi")
        ]
        vals = np.column_stack(
            [fcst_df[col].to_numpy().astype(float) for col in cols]
        )
        rescaled = mean_arr[:, np.newaxis] + (vals - mean_arr[:, np.newaxis]) * sigma_tgt[:, np.newaxis]
        fcst_df = ufp.assign_columns(fcst_df, cols, rescaled)
    return fcst_df


def _rescale_interval_columns_narwhals(
    fcst_df: DFType,
    model_names: List[str],
    level: List[Union[int, float]],
    sigma_tgt: np.ndarray,
) -> DFType:
    """Single narwhals with_columns call: one expression per interval column."""
    sigma_col = "__sigma_tgt__"
    fcst_df = ufp.copy_if_pandas(fcst_df, deep=False)
    fcst_df = ufp.assign_columns(fcst_df, sigma_col, sigma_tgt)
    ndf = nw.from_native(fcst_df, eager_only=True)
    exprs = []
    for model_name in model_names:
        for lv in level:
            for direction in ("lo", "hi"):
                col = f"{model_name}-{direction}-{lv}"
                offset = nw.col(col) - nw.col(model_name)
                exprs.append(
                    (nw.col(model_name) + offset * nw.col(sigma_col)).alias(col)
                )
    ndf = ndf.with_columns(*exprs).drop(sigma_col)
    return nw.to_native(ndf)
