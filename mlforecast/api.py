...
__all__ = ['Frame', 'validate_data_format', 'read_data', 'fcst_from_config', 'perform_backtest', 'parse_config',
           'setup_client', 'run_forecast']
...
from .data_model import Backtest, Cluster, Config, Data, DataFormat, Features, _available_tfms
...
# Cell
def fcst_from_config(config: Config):
    ...
    if config.local is not None:
        module_name, model_cls = config.local.model.name.rsplit('.', maxsplit=1)
        module = importlib.import_module(module_name)
        model = getattr(module, model_cls)(**(config.local.model.params or {}))
        fcst = Forecast(model, flow_config)
    elif config.distributed is not None:  # mypy can't see the validation for having either local or distributed
        import mlforecast.distributed.models

        model_cls = getattr(mlforecast.distributed.models, config.distributed.model.name)  # type: ignore
        model = model_cls(config.distributed.model.params or {})  # type: ignore
        fcst = DistributedForecast(model, flow_config)  # type: ignore
