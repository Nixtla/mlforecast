...
def _mape(y_true, y_pred, ids, _dates, weight_series=None):
    ...
    if weight_sum <= 0:
        warnings.warn(
            "Total weight is zero, cannot compute weighted MAPE.", UserWarning
        )
        return float("nan")
    ...
    def setup(
        ...
        Args:
            ...
            step_size (int, optional): Step size between each cross validation window. If None it will be equal to `h`.
                Defaults to None.
            params (dict, optional): Parameters to be passed to the LightGBM Datasets and Boosters. Defaults to None.
            ...
            categorical_feature (list of str, optional): Names of the features that LightGBM should treat as categorical.
                These can be features generated by this object, e.g. date features such as 'dayofweek'.
                This replaces LightGBM's inference rather than adding to it, so if your input has
                columns with a categorical dtype you have to list them here as well. An empty list
                means no categorical features. Defaults to None, which lets LightGBM infer them
                from the categorical dtypes of the input.
        ...
        for _, train, valid in splits:
            ...
            booster_params = {**self.params, "num_threads": self.bst_threads}
            ds = lgb.Dataset(
                prep.drop(
                ...
                categorical_feature=(
                    "auto" if categorical_feature is None else categorical_feature
                ),
            ).construct()

            bst = lgb.Booster(booster_params, ds)
