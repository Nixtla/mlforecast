...
dependency cost cancels in the delta. `baseline_rss_mb` is the control -- it
measures identical panel-building code on both sides, so if it moves, the runner
was noisy and the rest of the table is not worth reading.
"""
...
#: baseline drift above this means the two runs aren't comparable
...
def _baseline(data):
    """The panel-only baseline, identical across configs in a run."""
...
def _rows(base, pr):
    for name in sorted(pr):
        ...
        over = pr[name]["over_baseline_mb"]
        if name not in base:
            yield f"| `{name}` | new | {new:.0f} | | | {over:+.0f} |"
            continue
        ...
        yield (
            f"| `{name}` | {old:.0f} | {new:.0f} | {delta:+.0f}{flag} "
            f"| {pct:+.1f}% | {over:+.0f} |"
        )
...
def report(base, pr):
    ...
    if not base:
        ...
        out.append("| config | peak RSS (MB) | over baseline (MB) |")
        out.append("|---|--:|--:|")
        out += [
            f"| `{n}` | {v['peak_rss_mb']:.0f} | {v['over_baseline_mb']:+.0f} |"
            for n, v in sorted(pr.items())
        ]
        return "\n".join(out)

    base_ctl, pr_ctl = _baseline(base), _baseline(pr)
    ...
    if abs(drift) > CONTROL_DRIFT_MB:
        out.append(
            f"\n⚠️ **Noisy run**: the data baseline itself moved {drift:+.0f} MB "
            f"({base_ctl:.0f} → {pr_ctl:.0f} MB). It measures identical code on both "
            "sides, so the deltas below are not trustworthy."
        )
    out.append("")
    out.append("| config | base (MB) | PR (MB) | Δ MB | Δ % | PR over baseline (MB) |")
    out.append("|---|--:|--:|--:|--:|--:|")
    out += list(_rows(base, pr))
    out.append("")
    out.append(
        f"Data baseline (control): {base_ctl:.0f} → {pr_ctl:.0f} MB ({drift:+.0f}). "
