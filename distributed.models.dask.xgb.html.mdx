...
| n_estimators | int | 100 | Number of gradient boosted trees. Equivalent to number of boosting<br/>rounds. |
...
| sampling_method | Optional | None | Sampling method. Used only by `gpu_hist` tree method.<br/> - `uniform`: select random training instances uniformly.<br/> - `gradient_based` select random training instances with higher probability when<br/> the gradient and hessian are larger. (cf. CatBoost) |
...
| predictor | Optional | None | Force XGBoost to use specific predictor, available choices are \[cpu_predictor,<br/>gpu_predictor\]. |
...
| feature_types | Sequence | None | .. versionadded:: 1.7.0<br/><br/>Used for specifying feature types without constructing a dataframe. See<br/>:py:class:`DMatrix` for details. |
