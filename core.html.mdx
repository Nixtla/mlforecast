...
```python
fit_transform(data, id_col, time_col, target_col, static_features=None, dropna=True, keep_last_n=None, max_horizon=None, horizons=None, return_X_y=False, as_numpy=False, weight_col=None, horizon_features=None)
```
...
---- | ---- | ----------- | -------
`max_horizon` | <code>[Optional](#typing.Optional)[[int](#int)]</code> | Train models for all horizons 1 to max_horizon. | <code>None</code>
`horizons` | <code>[Optional](#typing.Optional)[[List](#typing.List)[[int](#int)]]</code> | Train models only for specific horizons (1-indexed).       Mutually exclusive with max_horizon. | <code>None</code>
`horizon_features` | <code>[Optional](#typing.Optional)[[Dict](#typing.Dict)[[int](#int), [List](#typing.List)[[str](#str)]]]</code> | Mapping of 1-indexed horizons to the dynamic exogenous columns each horizon's model uses. | <code>None</code>

#### `TimeSeries.history_warmup`

```python
history_warmup(df, id_col, time_col, target_col, static_features=None, keep_last_n=None, weight_col=None, max_horizon=None, horizons=None, as_numpy=None, horizon_features=None, trim=True)
```
...
`id_col` | <code>[str](#str)</code> | Column that identifies each serie. | *required*
`time_col` | <code>[str](#str)</code> | Column that identifies each timestep. | *required*
`target_col` | <code>[str](#str)</code> | Column that contains the target. | *required*
`static_features` | <code>[Optional](#typing.Optional)[[List](#typing.List)[[str](#str)]]</code> | Names of the features that are static. | <code>None</code>
`keep_last_n` | <code>[Optional](#typing.Optional)[[int](#int)]</code> | Keep only these many records from each serie. When None it's inferred from the transforms, like in `fit_transform`. | <code>None</code>
`weight_col` | <code>[Optional](#typing.Optional)[[str](#str)]</code> | Column that contains the sample weights. | <code>None</code>
`max_horizon` | <code>[Optional](#typing.Optional)[[int](#int)]</code> | Horizon the models were trained for (one model per horizon). Leave None for recursive models; when None, any value from a previous fit on this instance is preserved. | <code>None</code>
`horizons` | <code>[Optional](#typing.Optional)[[List](#typing.List)[[int](#int)]]</code> | Specific 1-indexed horizons the models were trained for. Mutually exclusive with max_horizon. | <code>None</code>
`as_numpy` | <code>[Optional](#typing.Optional)[[bool](#bool)]</code> | Whether prediction passes a numpy array to the model instead of a dataframe. When None, any value from a previous fit is preserved (False for a fresh instance). | <code>None</code>
`horizon_features` | <code>[Optional](#typing.Optional)[[Dict](#typing.Dict)[[int](#int), [List](#typing.List)[[str](#str)]]]</code> | Mapping of 1-indexed horizons to the dynamic exogenous columns the models were trained with. When None, any mapping from a previous fit is preserved. | <code>None</code>
`trim` | <code>[bool](#bool)</code> | Apply the `keep_last_n` trim after warming the transform state. `predict(new_df=...)` disables this to keep the full provided history. | <code>True</code>
...
Name | Type | Description | Default
---- | ---- | ----------- | -------
`df` | <code>[DataFrame](#utilsforecast.compat.DataFrame)</code> | New observations to append. | *required*
