...
methods. If your transformation stores large fitted state, also
implement `clone` to return an unfitted instance built from the same
arguments; the default copies the fitted state along. Here’s an example
