...
-   **Performance**: `RollingQuantile`, `ExpandingQuantile` and the
    ...
    aggregates.
-   **Memory**: pooled states keep the full training history regardless
    of `keep_last_n` (which only trims the per-series arrays used by
    local transforms), and each `predict` call backs up the pooled state
    per model to isolate recursive mutations. Expect higher peak memory
    with pooled transforms on large panels.
