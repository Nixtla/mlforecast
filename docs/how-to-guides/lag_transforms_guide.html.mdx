---
title: Lag transformations | MLForecast
---
...
## Cross-series features: `global_` and `groupby`
...
series) or `groupby=["col", ...]` (compute within a static-feature
group). This is useful when a single series has too little history to
support per-series rolling statistics, or when cross-series aggregates
carry more signal than any single series’ lag.
...
bucket. See the [Pooled lag transforms](./pooled_lag_transforms.html)
