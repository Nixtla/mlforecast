...
The flow above ran `MLForecast.preprocess` because we needed the
training features. If your models are already trained – for example you
just unpickled them in a serving process, or trained them yourself like
above – computing the features again just to restore the internal state
would be wasted work. `MLForecast.history_warmup` is a cheaper
alternative for that case: it runs the same state-building steps as
`preprocess` (target transforms, lag transform buffers, pooled
aggregates, feature order) but skips materializing the features
dataframe. Like `fit`, it also keeps only the history required for the
updates (`keep_last_n`, explicit or inferred from the transforms).
...
After warming up, `MLForecast.update` and `MLForecast.predict` can be
used directly – without ever calling `fit` or `preprocess` – and produce
the same forecasts as before.
