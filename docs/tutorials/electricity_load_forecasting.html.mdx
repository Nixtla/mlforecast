---
title: Electricity Load Forecast | MLForecast
---
...
Then we can instantiate the `MLForecast` class with the models we want
to try alongside `target_transforms`, `lags`, `lag_transforms`, and
`date_features`. All these features are applied to the models we
selected.
...
is behaving, yet in order to assess the performance we need to evaluate
them through metrics. For that we use the
...
The `predict` method returns a DataFrame with the predictions for each
model (`lasso` and `lgbm`) alongside the prediction thresholds. The
