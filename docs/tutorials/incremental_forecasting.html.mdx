---
title: Incremental Forecast generation
---

This tutorial provides a practical framework for developing
**incremental forecasting systems** using `MLForecast`. It walks through
...
parameters. In `MLForecast`, the `update()` method provides this
...
> 📘 *Note:* The dataset is loaded using `datasetsforecast` package,
> which automatically structures the data in the expected format for
> MLForecast - columns `unique_id`, `ds` (date), and `y` (target value).
...
retraining the model. This helps us understand how `update()` method can
...
1.  Nixtla Team. (2024).
    *[MLForecast](https://nixtlaverse.nixtla.io/mlforecast): Scalable
